...
class RecordCallback(MinecraftCallback):
    ...
    def _save_episode(self):
        ...
        if self.record_origin_observation:
            output_origin_path = self.record_path / f'session_{self.session_id}_episode_{self.episode_id}.npy'
        ...
        if self.record_actions: # assert self.actions>0 sense self.frame > 0
            output_action_path = self.record_path / f'session_{self.session_id}_episode_{self.episode_id}_action.json'
        ...
        if self.record_infos: # assert self.actions>0 sense self.frame > 0
            output_info_path = self.record_path / f'session_{self.session_id}_episode_{self.episode_id}_info.json'
